...
export class JogadoresService {
  ...
  public async criarJogador(criarJogadorDto: CriarJogadorDto): Promise<IJogador> {
    if (await this.jogadorModel.findOne({email: criarJogadorDto.email}).exec()) {
      throw new BadRequestException(`Jogado com e-mail ${criarJogadorDto.email} já existe`)
    }

    return new this.jogadorModel(criarJogadorDto).save()
  }

  public async atualizarJogador(id: string, atualizarJogadorDto: AtualizarJogadorDto): Promise<IJogador> {
