...
export class CategoriasService {
  ...
  @Inject('JogadoresService') private readonly jogadoresService: JogadoresService
  
  public async criarCategoria(criarCategoriaDto: CriarCategoriaDto): Promise<ICategoria> {
  ...
  public async atualizarCategoria(categoria: string, atualizarCategoriaDto: AtualizarCategoriaDto): Promise<ICategoria> {
    const categoriaEncontrada = await this.categoriaModel.findOne({ categoria }).exec()
  ...
  public async atribuirCategoriaJogador(categoria: string, idJogador: string): Promise<ICategoria> {
    const categoriaEncontrada = await this.categoriaModel.findOne({ categoria }).exec()

    if (!categoriaEncontrada) {
      throw new BadRequestException('Categoria não encontrada ao atribuir um jogador')
    }

    if (categoriaEncontrada.jogadores.find(jogador => jogador.toJSON() === idJogador)) {
      throw new BadRequestException('Jogador já cadastrado nessa categória')
    }

    categoriaEncontrada.jogadores.push(await this.jogadoresService.consultarJogador(idJogador))
    await categoriaEncontrada.updateOne(categoriaEncontrada).exec()
    
    return this.categoriaModel.findOne({ categoria }).populate('jogadores').exec()
  }
